            ...
            @foreach($clases as $clase)
              <tr class="hover:bg-gray-50">
                ...
                <td class="py-2 px-4 border">
                  {{ $clase->fecha ? \Carbon\Carbon::parse($clase->fecha)->format('d/m/Y') : 'Sin fecha' }}<br>
                  {{ $clase->horario ?? 'Sin horario' }}
                </td>
                <td class="py-2 px-4 border font-semibold">{{ $clase->instructor ?? 'Sin asignar' }}</td>
                <td class="py-2 px-4 border">
                    ...
                    @if(!$reserva)
                      ...
                      <form action="{{ route('reserva.store') }}" method="POST" class="inline-block">
                        ...
                        <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700
                           @if($clase->capacidad_max && $clase->reservas_count >= $clase->capacidad_max) opacity-50 cursor-not-allowed bg-gray-400 @endif"
                          @if($clase->capacidad_max && $clase->reservas_count >= $clase->capacidad_max) disabled @endif>
                          <i class="fas fa-bookmark mr-1"></i> Reservar
                        </button>
                      </form>
                    @else
                      <!-- Botón cancelación junto a Reservada -->
                      <span class="px-4 py-2 bg-green-100 text-green-800 rounded-inline-flex items-center">
                        <i class="fas fa-check mr-1"></i> Reservada
                      </span>
                      <form action="{{ route('reserva.destroy', $reserva->id) }}" method="POST" class="inline-block ml-2">
                        @csrf
                        @method('DELETE')
